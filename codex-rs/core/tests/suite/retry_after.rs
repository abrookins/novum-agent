...
const CONNECTION_RETRY_TEST_SUBPROCESS_ENV_VAR: &str = "CODEX_CONNECTION_RETRY_TEST_SUBPROCESS";
const CONNECTION_RETRY_TEST_NAME: &str = "suite::retry_after::connection_failures_increment_retry_telemetry_without_consuming_retry_budget";
...
async fn connection_failures_increment_retry_telemetry_without_consuming_retry_budget() -> Result<()>
{
    ...
    if std::env::var_os(CONNECTION_RETRY_TEST_SUBPROCESS_ENV_VAR).is_none() {
        let mut command = Command::new(std::env::current_exe()?);
        command.arg("--exact").arg(CONNECTION_RETRY_TEST_NAME);
        for &key in codex_network_proxy::PROXY_ENV_KEYS {
            command.env_remove(key);
        }
        command
            .env(CONNECTION_RETRY_TEST_SUBPROCESS_ENV_VAR, "1")
        ...
        assert!(
            ...
            "subprocess test `{CONNECTION_RETRY_TEST_NAME}` failed\nstdout:\n{}\nstderr:\n{}",
