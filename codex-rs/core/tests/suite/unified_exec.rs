...
fn assert_recoverable_output_guidance(output: &str) {
    assert_regex_match(
        r"Full output is available as `out_[0-9a-f]{32}`\. Use tool_output\.read or tool_output\.search\.",
        output,
    );
    assert_eq!(output.matches("Full output is available as").count(), 1);
}
...
async fn exec_command_clamps_model_requested_max_output_tokens_to_policy() -> Result<()> {
    ...
    assert!(
        approx_token_count(&output_text) <= 50,
        "output exceeded the configured 50-token policy: {output_text:?}"
    );
    assert_eq!(output_text.matches("tokens truncated").count(), 1);
    assert_recoverable_output_guidance(&output_text);
...
async fn write_stdin_clamps_model_requested_max_output_tokens_to_policy() -> Result<()> {
    ...
    assert!(
        approx_token_count(&stdin_output_text) <= 50,
        "output exceeded the configured 50-token policy: {stdin_output_text:?}"
    );
    assert_eq!(stdin_output_text.matches("tokens truncated").count(), 1);
    assert_recoverable_output_guidance(&stdin_output_text);
...
"#
    ...
    assert!(
        approx_token_count(&output_text) <= 100,
        "output exceeded the requested 100-token limit: {output_text:?}"
    );
    assert_eq!(output_text.matches("tokens truncated").count(), 1);
    assert_recoverable_output_guidance(&output_text);
    assert_eq!(
