...
async fn unified_exec_zsh_fork_guardian_reviews_persistent_terminal_in_current_turn() -> Result<()>
{
    ...
    // Denied reads reject writes to an escalated terminal before approval.
    // Exercise cross-turn Guardian review with a reusable launch policy.
    let permission_profile = restrictive_workspace_write_profile();
    let rules = r#"prefix_rule(pattern=["touch"], decision="prompt")"#.to_string();
    ...
    // New denied-read restrictions reject input before Guardian review because
    // they cannot be applied to an existing terminal. Change the environment
    // without adding denied reads so this test exercises the current reviewer.
